...
import {
  ...
  CheckCircle2, Circle, ArrowUpRight, FileText, Plus, Coffee
} from 'lucide-react'
...
export default function EmployeePortal() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (tasksData.status) setTasks(tasksData.data || [])
        if (empData.status) {
          const emps = empData.data || []
          const myEmp = emps.find((e: any) => e.userId === user?.id) || emps[0]
          setEmployee(myEmp)
          if (myEmp) {
            const attRes = await fetch(`/api/attendance?employeeId=${myEmp.id}`)
            const attData = await attRes.json()
            if (attData.status) setAttendance(attData.data || [])
            const todayAtt = (attData.data || []).find((a: any) => new Date(a.date).toDateString() === new Date().toDateString())
            if (todayAtt?.checkin) { setCheckedIn(true); setCheckinTime(formatTime(todayAtt.checkin)) }
          }
        }
      } catch (err) {
  ...
  const handleCheckIn = async () => {
    ...
    try {
      const res = await fetch('/api/attendance', {
        ...
        body: JSON.stringify({ employeeId: employee.id, type: checkedIn ? 'checkout' : 'checkin' }),
      })
      const data = await res.json()
      if (data.status) {
        if (!checkedIn) { setCheckedIn(true); setCheckinTime(formatTime(new Date().toISOString())); toast.success('Checked in successfully') }
        else { setCheckedIn(false); setCheckinTime(null); toast.success('Checked out successfully') }
      }
  ...
  const handleApplyLeave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!employee) return
    try {
      // Using the existing leaves endpoint
      toast.success('Leave application submitted')
      setLeaveDialog(false)
    } catch { toast.error('Failed to apply for leave') }
  }
    ...
    <div className="min-h-screen bg-[#0b0b0b] pt-20 md:pt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={handleLogout} variant="ghost" className="text-gray-500 hover:text-red-400 text-sm"><LogOut className="w-4 h-4 mr-2" />Logout</Button>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-56 shrink-0">
            <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-2 flex md:flex-col gap-1 overflow-x-auto md:overflow-x-visible sticky top-24">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setEmployeeTab(tab.id)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                    employeeTab === tab.id ? 'bg-[#59ff00]/10 text-[#59ff00]' : 'text-gray-500 hover:text-white hover:bg-[#1a1a1a]'
                  }`}>
                  {tab.icon} {tab.label}
                </button>
              ))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div><div className="text-gray-500 text-xs">Today&apos;s Status</div><div className="text-green-400 font-semibold">{checkedIn ? 'Checked In' : 'Not Checked In'}</div></div>
                      </div>
                      {checkinTime && <div className="text-gray-600 text-xs mt-2 ml-13">Since {checkinTime}</div>}
                    </div>
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-yellow-500/10 flex items-center justify-center"><AlertCircle className="w-5 h-5 text-yellow-400" /></div>
                        <div><div className="text-gray-500 text-xs">Pending Tasks</div><div className="text-yellow-400 font-semibold text-lg">{pendingTasks}</div></div>
                      </div>
                    </div>
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center"><Play className="w-5 h-5 text-blue-400" /></div>
                        <div><div className="text-gray-500 text-xs">In Progress</div><div className="text-blue-400 font-semibold text-lg">{inProgressTasks}</div></div>
                      </div>
                    </div>
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg bg-[#59ff00]/10 flex items-center justify-center"><CheckSquare className="w-5 h-5 text-[#59ff00]" /></div>
                        <div><div className="text-gray-500 text-xs">Completed</div><div className="text-[#59ff00] font-semibold text-lg">{completedTasks}</div></div>
                      </div>
                    </div>
                  </div>
                  <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      <button onClick={handleCheckIn} className={`flex flex-col items-center gap-2 rounded-lg p-4 transition-colors ${
                        checkedIn ? 'bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20' : 'bg-[#1a1a1a] border border-[#2a2a2a] text-gray-400 hover:text-[#59ff00] hover:border-[#59ff00]/30'
                      }`}>
                        {checkedIn ? <Square className="w-5 h-5" /> : <Play className="w-5 h-5" />}
                        <span className="text-xs">{checkedIn ? 'Check Out' : 'Check In'}</span>
                      </button>
                      <button onClick={() => setEmployeeTab('tasks')} className="flex flex-col items-center gap-2 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-4 text-gray-400 hover:text-[#59ff00] hover:border-[#59ff00]/30 transition-colors">
                        <CheckSquare className="w-5 h-5" /><span className="text-xs">View Tasks</span>
                      </button>
                      <button onClick={() => setLeaveDialog(true)} className="flex flex-col items-center gap-2 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-4 text-gray-400 hover:text-[#59ff00] hover:border-[#59ff00]/30 transition-colors">
                        <CalendarDays className="w-5 h-5" /><span className="text-xs">Apply Leave</span>
                      </button>
                      <button onClick={() => setEmployeeTab('salary')} className="flex flex-col items-center gap-2 bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-4 text-gray-400 hover:text-[#59ff00] hover:border-[#59ff00]/30 transition-colors">
                        ...
                        {tasks.filter(t => t.status === 'in_progress').map(task => (
                          <div key={task.id} className="bg-[#151515] border border-[#2a2a2a] rounded-lg p-3 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <div><div className="text-white text-sm">{task.title}</div><div className="text-gray-600 text-xs">Due: {task.dueDate ? formatDate(task.dueDate) : 'N/A'}</div></div>
                            </div>
              ...
              {employeeTab === 'attendance' && (
                <div>
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    <Button onClick={handleCheckIn} className={checkedIn ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-[#59ff00] text-black hover:bg-[#59ff00]/90'}>
                      {checkedIn ? <><Square className="w-4 h-4 mr-2" />Check Out</> : <><Play className="w-4 h-4 mr-2" />Check In</>}
                    </Button>
                    {checkinTime && <span className="text-gray-500 text-sm">Checked in at {checkinTime}</span>}
                  </div>
                  {/* Calendar View */}
                            ...
                            {attendance.map((att: any) => {
                              ...
                              return (
                                <tr key={att.id} className="border-t border-[#2a2a2a] hover:bg-[#1a1a1a]">
                                  ...
                                  <td className="px-4 py-3"><Badge className={`${statusColor[att.status] || ''} text-xs`}>{att.status}</Badge></td>
                                </tr>
                              )
                            })}
                          </tbody>
                  ...
                  ) : <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-8 text-center text-gray-600">No attendance records found</div>}
                </div>
              )}
                    ...
                    {['all', 'pending', 'in_progress', 'completed'].map(s => (
                      <button key={s} onClick={() => setTaskFilter(s)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize whitespace-nowrap transition-colors ${
                          taskFilter === s ? 'bg-[#59ff00]/10 text-[#59ff00] border border-[#59ff00]/20' : 'bg-[#1a1a1a] text-gray-500 hover:text-white border border-[#2a2a2a]'
                        }`}>{s === 'all' ? 'All Tasks' : s.replace('_', ' ')}</button>
                    ))}
                  </div>
                  {filteredTasks.length === 0 ? (
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-8 text-center text-gray-600">No tasks found</div>
                  ) : (
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex items-start gap-3 flex-1">
                              <button onClick={() => {
                                ...
                                {task.status === 'completed' ? <CheckCircle2 className="w-5 h-5 text-green-400" /> : task.status === 'in_progress' ? <Play className="w-5 h-5 text-blue-400" /> : <Circle className="w-5 h-5 text-gray-600" />}
                              </button>
                              <div>
                                <h3 className={`text-sm font-semibold ${task.status === 'completed' ? 'text-gray-500 line-through' : 'text-white'}`}>{task.title}</h3>
                                {task.description && <p className="text-gray-600 text-xs mt-1">{task.description}</p>}
                                <div className="flex items-center gap-3 mt-2 text-xs text-gray-600">
                                  {task.dueDate && <span className="flex items-center gap-1"><CalendarDays className="w-3 h-3" />{formatDate(task.dueDate)}</span>}
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {task.status !== 'completed' && (
                                <Select onValueChange={(val) => handleTaskStatus(task.id, val)}>
                                  <SelectTrigger className="w-7 h-7 bg-[#1a1a1a] border-[#2a2a2a] p-0"><ChevronDown className="w-3 h-3 text-gray-500" /></SelectTrigger>
              ...
              {employeeTab === 'salary' && (
                <div>
                  ...
                  {employee?.salarySlips?.length > 0 ? (
                    <div className="space-y-3">
                      {employee.salarySlips.map((slip: any) => (
              ...
              {employeeTab === 'leaves' && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Button onClick={() => setLeaveDialog(true)} size="sm" className="bg-[#59ff00] text-black hover:bg-[#59ff00]/90"><Plus className="w-4 h-4 mr-1" />Apply Leave</Button>
                  </div>
                  {/* Leave Balance */}
                  ...
                  {employee?.leaves?.length > 0 ? (
                          ...
                          <tbody>
                            {employee.leaves.map((leave: any) => (
                              <tr key={leave.id} className="border-t border-[#2a2a2a] hover:bg-[#1a1a1a]">
                                ...
                                <td className="px-4 py-3 text-gray-400">{Math.ceil((new Date(leave.endDate).getTime() - new Date(leave.startDate).getTime()) / 86400000) + 1}</td>
                                <td className="px-4 py-3 text-gray-400 max-w-[150px] truncate">{leave.reason || '-'}</td>
                                <td className="px-4 py-3"><Badge className={`${statusColor[leave.status] || ''} text-xs`}>{leave.status}</Badge></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl p-8 text-center text-gray-600">No leave records found</div>
                  )}
                </div>
              )}
            </motion.div>
      ...
      <Dialog open={leaveDialog} onOpenChange={setLeaveDialog}>
        <DialogContent className="bg-[#151515] border-[#2a2a2a] text-white max-w-md">
          <DialogHeader><DialogTitle className="text-[#59ff00]">Apply for Leave</DialogTitle></DialogHeader>
          <form onSubmit={handleApplyLeave} className="space-y-4">
            <div><label className="text-sm text-gray-400 mb-1 block">Leave Type</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-sm text-gray-400 mb-1 block">Start Date</label><Input type="date" name="startDate" required className="bg-[#1a1a1a] border-[#2a2a2a] text-white" /></div>
              <div><label className="text-sm text-gray-400 mb-1 block">End Date</label><Input type="date" name="endDate" required className="bg-[#1a1a1a] border-[#2a2a2a] text-white" /></div>
            </div>
            <div><label className="text-sm text-gray-400 mb-1 block">Reason</label><Textarea name="reason" placeholder="Enter reason for leave..." className="bg-[#1a1a1a] border-[#2a2a2a] text-white" /></div>
            <Button type="submit" className="w-full bg-[#59ff00] text-black hover:bg-[#59ff00]/90 font-semibold">Submit Leave Request</Button>
          </form>
        </DialogContent>
      </Dialog>
    </div>
...
function SalarySlipCard({ slip }: { slip: any }) {
  ...
  return (
    <div className="bg-[#151515] border border-[#2a2a2a] rounded-xl overflow-hidden">
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between p-4 hover:bg-[#1a1a1a] transition-colors">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-lg bg-[#59ff00]/10 flex items-center justify-center"><FileText className="w-5 h-5 text-[#59ff00]" /></div>
