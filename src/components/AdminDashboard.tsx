...
import {
  ...
  FolderOpen, Clock4, UsersRound, Lock, Printer, Download,
  Share2, Palette, BellRing, Zap, ArrowRight, LayoutGrid,
  Filter, Megaphone, Webhook, FormInput, Database, Gauge,
  TrendingDown, Timer, HandCoins, CalendarClock,
  FileSpreadsheet, Leaf
} from 'lucide-react'
  ...
  sections: [
    {
      ...
      items: [
        ...
        { key: 'crm-companies', label: 'Companies', icon: Building2 },
        { key: 'crm-pipelines', label: 'Pipelines', icon: Target },
        { key: 'crm-pipeline', label: 'Pipeline', icon: Filter },
        { key: 'crm-kanban', label: 'Kanban Board', icon: Kanban },
        { key: 'crm-calendar', label: 'Task Calendar', icon: CalendarCheck },
        { key: 'crm-forms', label: 'Forms', icon: ClipboardList },
        { key: 'crm-imports', label: 'Imports', icon: Import },
        { key: 'crm-conversations', label: 'Conversations', icon: MessagesSquare },
        { key: 'crm-lead-sources', label: 'Lead Sources', icon: Megaphone },
        { key: 'crm-integrations', label: 'Integrations', icon: Webhook },
        { key: 'crm-email-templates', label: 'Email Templates', icon: MailOpen },
        { key: 'crm-email-sequences', label: 'Email Sequences', icon: MailPlus },
      ],
    },
    {
      label: 'REPORTS',
    ...
    },
    {
      ...
      items: [
        ...
        { key: 'crm-quotes', label: 'Quotes', icon: Receipt },
        { key: 'crm-invoices', label: 'Invoices', icon: FileText },
        { key: 'crm-billing', label: 'Billing', icon: CreditCard },
        { key: 'crm-sales-calendar', label: 'Calendar', icon: CalendarRange },
      ],
    },
  ],
}

const HRM_GROUP: SidebarGroup = {
  id: 'hrm',
  label: 'HRM',
  icon: Briefcase,
  sections: [
...
export default function AdminDashboard() {
  ...
  // ─── Quotation Template & Company ───────────────────────
  const [quotationTemplate, setQuotationTemplate] = useState<'modern' | 'minimal' | 'corporate' | 'premium'>('modern')
  const [companyCustomization, setCompanyCustomization] = useState({
    logo: '', name: 'Urban Kitchen Manufacturing & Solutions', address: 'Sector 12, Industrial Area, New Delhi',
    contact: '+91-9876543210', email: 'sales@urbankitchen.com', website: 'www.urbankitchen.com',
    gstNumber: '07AABCU9603R1ZM', signature: '', terms: '1. Prices are exclusive of freight & insurance charges unless stated otherwise.\n2. GST @18% applicable as per government norms.\n3. 50% advance payment with order, balance before dispatch.\n4. Delivery subject to confirmation at the time of order.\n5. Goods once sold will not be taken back.\n6. Subject to Delhi jurisdiction.\n7. This quotation is valid for 30 days from the date of issue.',
    brandColor: '#59ff00', footerNotes: 'Thank you for your business!',
  })
  ...
  const renderTabContent = () => {
    ...
    switch (adminTab) {
      ...
      // CRM tabs
      case 'crm-dashboard': return renderCrmDashboard()
      ...
      // HRM tabs
      case 'hrm-dashboard': return renderHrmDashboard()
        ...
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 py-1 px-2 space-y-0.5 overflow-y-auto custom-scrollbar">
            ...
            {(() => {
              const filteredCrm = {
                ...CRM_GROUP,
                sections: CRM_GROUP.sections
                  .map(s => ({ ...s, items: filterSidebarItems(s.items, sidebarSearch) }))
                  .filter(s => s.items.length > 0),
              }
              if (sidebarSearch && filteredCrm.sections.flatMap(s => s.items).length === 0) return null
              return (
                <div className="mt-1">
                  <button
                    onClick={() => setCrmExpanded(!effectiveCrmExpanded)}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium
                      ${isCrmTab ? 'text-[#59ff00]' : 'text-gray-400 hover:text-white'} hover:bg-white/5`}
                  ...
                  >
                    ...
                    {!sidebarCollapsed && (
                      <>
                        ...
                        <motion.div animate={{ rotate: effectiveCrmExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
                    ...
                    {effectiveCrmExpanded && !sidebarCollapsed && (
                      ...
                      >
                        <div className="ml-3 pl-3 border-l border-[#2a2a2a] space-y-0.5 py-1">
                          {filteredCrm.sections.map((section, si) => (
                            <div key={si}>
                              {section.label && (
                                <p className="text-gray-600 text-[10px] font-bold tracking-widest px-3 py-1.5 mt-1">{section.label}</p>
                              )}
                              {section.items.map(({ key, label, icon: ItemIcon }) => (
                                <button
                                  key={key}
                                  onClick={() => { setAdminTab(key); setMobileOpen(false) }}
                                  className={`w-full flex items-center gap-2.5 px-3 py-1.5 rounded-md transition-all duration-200 text-xs font-medium
                                    ${adminTab === key
            ...
            {(() => {
              const filteredHrm = {
                ...HRM_GROUP,
                sections: HRM_GROUP.sections
                  .map(s => ({ ...s, items: filterSidebarItems(s.items, sidebarSearch) }))
                  .filter(s => s.items.length > 0),
              }
              if (sidebarSearch && filteredHrm.sections.flatMap(s => s.items).length === 0) return null
              return (
                <div className="mt-1">
                  <button
                    onClick={() => setHrmExpanded(!effectiveHrmExpanded)}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium
                      ${isHrmTab ? 'text-[#59ff00]' : 'text-gray-400 hover:text-white'} hover:bg-white/5`}
                    title={sidebarCollapsed ? 'HRM' : undefined}
                  >
                    <Briefcase className="w-5 h-5 flex-shrink-0" />
                    {!sidebarCollapsed && (
                      <>
                        <span className="flex-1 text-left">HRM</span>
                        <motion.div animate={{ rotate: effectiveHrmExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
                          <ChevronDown className="w-4 h-4" />
                        </motion.div>
                      </>
                    )}
                  ...
                  <AnimatePresence>
                    {effectiveHrmExpanded && !sidebarCollapsed && (
                      <motion.div
                        initial={{ height: 0, opacity: 0 }}
                        animate={{ height: 'auto', opacity: 1 }}
                        exit={{ height: 0, opacity: 0 }}
                      ...
                      >
                        <div className="ml-3 pl-3 border-l border-[#2a2a2a] space-y-0.5 py-1">
                          {filteredHrm.sections.map((section, si) => (
                            <div key={si}>
                              {section.label && (
                                <p className="text-gray-600 text-[10px] font-bold tracking-widest px-3 py-1.5 mt-1">{section.label}</p>
                              )}
              ...
              {(() => {
                ...
                return (
                  <div className="mt-1">
                    <button
                      onClick={() => setCrmExpanded(!effectiveCrmExpanded)}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium ${isCrmTab ? 'text-[#59ff00]' : 'text-gray-400 hover:text-white'} hover:bg-white/5`}
                    >
                      <PieChartIcon className="w-5 h-5 flex-shrink-0" />
                      <span className="flex-1 text-left">CRM</span>
                      <motion.div animate={{ rotate: effectiveCrmExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
              ...
              {(() => {
                ...
                return (
                  <div className="mt-1">
                    <button
                      onClick={() => setHrmExpanded(!effectiveHrmExpanded)}
                      className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 text-sm font-medium ${isHrmTab ? 'text-[#59ff00]' : 'text-gray-400 hover:text-white'} hover:bg-white/5`}
                    >
                      <Briefcase className="w-5 h-5 flex-shrink-0" />
                      <span className="flex-1 text-left">HRM</span>
                      <motion.div animate={{ rotate: effectiveHrmExpanded ? 180 : 0 }} transition={{ duration: 0.2 }}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="flex items-center justify-between px-4 lg:px-6 py-3 border-b border-[#2a2a2a] bg-[#0b0b0b]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-semibold text-lg capitalize">{adminTab.replace(/_/g, ' ').replace(/crm-|hrm-/g, '')}</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 bg-[#181818] border border-[#2a2a2a] rounded-lg px-3 py-1.5 w-64">
              ...
              />
              {headerSearch && (
                <button onClick={() => setHeaderSearch('')} className="text-gray-500 hover:text-white">
                  <X className="w-3 h-3" />
                </button>
              )}
            </div>
            ...
            {/* Notification Bell */}
            <button className="relative w-9 h-9 rounded-lg bg-[#181818] border border-[#2a2a2a] flex items-center justify-center text-gray-400 hover:text-white hover:border-[#59ff00]/30 transition-all">
      ...
      <Dialog open={quotationDialog} onOpenChange={setQuotationDialog}>
        <DialogContent className="bg-[#181818] border-[#2a2a2a] text-white max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {/* Template Selector */}
            <div>
              <h3 className="text-[#59ff00] text-sm font-semibold mb-2 flex items-center gap-2"><Palette className="w-4 h-4" /> Template Style</h3>
              <div className="grid grid-cols-4 gap-2">
                {(['modern', 'minimal', 'corporate', 'premium'] as const).map(t => (
                  <button key={t} onClick={() => setQuotationTemplate(t)}
            ...
            <details className="group">
              <summary className="text-sm text-gray-400 cursor-pointer hover:text-white flex items-center gap-2">
                <Building2 className="w-4 h-4" /> Company Customization
          ...
          <DialogFooter className="mt-4 gap-2">
            ...
            <Button variant="outline" onClick={() => window.print()} className="border-[#2a2a2a] text-gray-300 hover:text-white">
